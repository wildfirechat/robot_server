...
public class BullshitGenerator {
    private static String famous[] = {"爱迪生a，天才是百分之一的勤奋加百分之九十九的汗水。b",
    ...
    private static String getNextBosh() {
        return bosh[(int)(random.nextFloat()*bosh.length)];
    }
    private static String getFamous() {
        String a = famous[(int)(random.nextFloat()*famous.length)];
        a = a.replace("a", before[(int)(random.nextFloat()*before.length)]);
        a = a.replace("b", after[(int)(random.nextFloat()*after.length)]);
        return a;
    }
    private static String getNewLine() {
        return "\r\n    ";
    }
    public static String bullshit(String topic) {
        StringBuilder sb = new StringBuilder(topic);

        sb.append(getNewLine());
        int newLineLen = 0;
        boolean famousStart = false;

        while (sb.length() < 6000) {
            int branch = (int)(Math.random() * 100);
            if (branch < 5 || newLineLen > 800) { //超过800换行
                if (newLineLen == 0) {
                    continue;
                } else {
                    if (newLineLen < 200) { //小于200就不要换行了，太少了
                        continue;
                    }
                    sb.append(getNewLine());
                    newLineLen = 0;
                    famousStart = (int)(Math.random() * 100) > 40; //40%的段落以名人名言开头
                }
            } else if(branch < 20) {
                String f = getFamous();
                newLineLen += f.length();
                ...
                String b = getNextBosh();
                newLineLen += b.length();
                sb.append(b);
            }
        }
        String article = sb.toString();
        article = article.replaceAll("x", topic);
        return article;
    }

    public static void main(String[] args) {
        String article = bullshit("学生会退会");
